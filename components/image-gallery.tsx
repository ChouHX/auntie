...
import { resolveGalleryImageSources } from "@/lib/image-sources"
  ...
  const galleryImages = useMemo(
    () =>
      items
        ...
        .map((item, index) => {
          const sources = resolveGalleryImageSources(item)
...
function GalleryImageCard({ eager, image, onOpen }: GalleryImageCardProps) {
  return (
    <button
      ...
      onFocus={() => preloadGalleryImage(image.detailSrc)}
      onPointerEnter={() => preloadGalleryImage(image.detailSrc)}
      type="button"
    >
      <Image
