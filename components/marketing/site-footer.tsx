...
function SiteFooter() {
  ...
  const contactEmail =
    contactSettings?.contactEmail || companyIdentity.contactEmail
  ...
  return (
    <footer className="relative overflow-hidden border-t border-blue-100/80 text-slate-700 transition-colors duration-300 dark:border-white/10 dark:text-slate-200">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-12">
          ...
          <section>
            ...
            <div className="mt-4 space-y-3 text-sm leading-7 text-slate-600 dark:text-slate-300">
              ...
              <div className="flex w-fit flex-col items-center gap-2.5">
                <img
                  alt="微信二维码"
                  className="size-24 rounded-xl bg-white object-cover p-1.5 shadow-sm ring-1 ring-blue-100 dark:ring-white/10"
                  loading="lazy"
                  src={contactQrImage}
                />
                <a
                  className="flex max-w-[12rem] items-center justify-center gap-1.5 text-center text-xs font-semibold break-all text-slate-700 transition hover:text-blue-700 dark:text-slate-200 dark:hover:text-blue-200"
        ...
        <div className="flex flex-col gap-4 border-t border-blue-200/80 pt-6 text-xs text-slate-500 md:flex-row md:items-start md:justify-between dark:border-white/10 dark:text-slate-400">
          <div className="space-y-1.5">
            <div>
              © 2026 {dict.common.brandName} | {dict.footer.rights}
            </div>
            <div className="max-w-xl leading-5">
              {companyIdentity.legalName}
            </div>
            <div className="max-w-xl leading-5">
              {language === "zh" ? "注册地址：" : "Registered Address: "}
              {companyIdentity.registeredOffice}
            </div>
          </div>
