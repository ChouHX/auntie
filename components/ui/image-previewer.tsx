...
function ImagePreviewer({
  ...
  const activeImageSrc = activeImage?.detailSrc || activeImage?.src
  const canNavigate = images.length > 1
  const isOpen = openIndex !== null && Boolean(activeImage)
  const shouldUseMobileLandscape = Boolean(activeImage?.mobileLandscape)
  ...
  useEffect(() => {
    ...
    indexesToPreload.forEach((index) => {
      ...
      const src = imageItem?.detailSrc || imageItem?.src
  ...
  return createPortal(
    ...
    >
      ...
      <div className="image-preview-chrome pointer-events-none absolute top-4 left-1/2 z-40 -translate-x-1/2 rounded-full border border-white/12 bg-white/10 px-4 py-2 text-sm font-semibold text-white shadow-2xl">
        {activeIndex + 1} / {images.length}
      </div>

      <Button
        aria-label="关闭大图"
        className="image-preview-chrome absolute top-4 right-4 z-50 rounded-full border-white/10 bg-white/10 text-white shadow-2xl hover:bg-white/18"
        onClick={requestClose}
        size="icon"
        type="button"
        variant="ghost"
      ...
      {canNavigate ? (
        <>
          <Button
            aria-label="上一张"
            className="image-preview-chrome absolute top-1/2 left-3 z-50 -translate-y-1/2 rounded-full border-white/10 bg-white/10 text-white shadow-2xl hover:bg-white/18 sm:left-5"
          ...
          <Button
            aria-label="下一张"
            className="image-preview-chrome absolute top-1/2 right-3 z-50 -translate-y-1/2 rounded-full border-white/10 bg-white/10 text-white shadow-2xl hover:bg-white/18 sm:right-5"
            onClick={() => goTo(activeIndex + 1)}
            size="icon-lg"
            type="button"
            variant="ghost"
      ...
      <div
        className={cn(
          "pointer-events-none relative z-30 flex min-h-0 flex-1 items-center justify-center px-4 py-20 sm:px-8",
          shouldUseMobileLandscape && "max-sm:px-0 max-sm:py-16"
        )}
      >
        <div
          className={cn(
            shouldUseMobileLandscape && "image-preview-stage-landscape"
          )}
        >
          <div
            className="image-preview-matte pointer-events-auto max-h-[calc(100dvh-10rem)] max-w-[calc(100dvw-2rem)] overflow-hidden rounded-2xl border border-white/20 p-2 shadow-[0_28px_90px_rgb(0_0_0/0.48)] sm:max-w-[calc(100dvw-4rem)] sm:p-3"
          ...
          >
            <img
              ...
              alt={activeImage.alt}
              className={cn(
                "image-preview-image block max-h-[calc(100dvh-12rem)] max-w-[calc(100dvw-3rem)] rounded-xl bg-white object-contain select-none sm:max-w-[calc(100dvw-6rem)]",
      ...
      <div className="image-preview-chrome fixed inset-x-0 bottom-4 z-50 mx-auto flex w-fit max-w-[calc(100dvw-1.5rem)] items-center gap-2 rounded-full border border-white/12 bg-white/10 px-2 py-2 text-white shadow-2xl">
        <Button
          aria-label="缩小"
